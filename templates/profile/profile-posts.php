      ...
      <header class="post__header">
        <?php if (!empty($post['original_id'])) : ?>
          <div class="post__author">
            <a class="post__author-link" href="profile.php?user_id=<?= $post['original_author_id'] ?>&active_tab=posts" title="Автор">
              <div class="post__avatar-wrapper post__avatar-wrapper--repost">
                <img class="post__author-avatar" width="60px" src="userpics/<?= $post['original_author_avatar'] ?>" alt="Аватар пользователя">
              </div>
        ...
        <div class="post__main">
          <?php if ($post['type'] === 'photo') : ?>
            <div class="post-photo__image-wrapper">
              <img src="uploads/<?= anti_xss($post['img']) ?>" alt="Фото от пользователя" width="760" height="396">
            </div>
          <?php elseif ($post['type'] === 'video') : ?>
            ...
            <div class="comments">
              <a class="comments__button button <?= ($comments_for_id === $post['id']) ? "visually-hidden" : ""; ?>" href="profile.php?comments_for=<?= $post['id'] ?>#post-<?= $post['id'] ?>">Показать комментарии</a>
              <?php if ($comments_for_id === $post['id']) : ?>
                <div class="comments__list-wrapper">
                    ...
                    <?php foreach ($comments[$post['id']] as $comment) : ?>
                      <li class="comments__item user">
                        <div class="comments__avatar">
                          <a class="user__avatar-link" href="profile.php?user_id=<?= $comment['user_id'] ?>&active_tab=posts">
                            <img class="comments__picture" width="40px" src="userpics/<?= anti_xss($comment['avatar']) ?>" alt="Аватар пользователя">
                          </a>
                        </div>
                        <div class="comments__info">
                          <div class="comments__name-wrapper">
                            <a class="comments__user-name" href="profile.php?user_id=<?= $comment['user_id'] ?>&active_tab=posts">
                          ...
                          <p class="comments__text" style="font-size:16px; color:black;text-align:left;font-weight:normal">
                  ...
                  <a class="comments__more-link" href="<?= $_SERVER['REQUEST_URI'] . "&show_comments=all#post-" .$post['id'] ?>">
              ...
              <div class="comments__my-avatar">
                <img class="comments__picture" width="40px+" src="userpics/<?= $user_data['avatar'] ?>" alt="Аватар пользователя">
              </div>
              ...
              <input type="hidden" value="<?= $post['user_id']?>" name="author_id" id="author_id">
