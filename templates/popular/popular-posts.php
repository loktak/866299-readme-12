        ...
        <article class="popular__post post post-<?= $post['icon_type'] ?>">
            ...
            <div class="post__main">
                ...
                <?php elseif ($post['icon_type'] === 'photo'): ?>
                    <div class="post-photo__image-wrapper">
                        <img src="uploads/<?= anti_xss($post['img']) ?>" alt="Фото от пользователя" width="360"
                             height="240">
                    </div>
                <?php elseif ($post['icon_type'] === 'link'): ?>
                    <div class="post-link__wrapper">
                        <a class="post-link__external" href="<?= anti_xss($post['link']) ?>" target="_blank"
                ...
                <?php elseif ($post['icon_type'] === 'video'): ?>
                    <div class="post-video__block">
                        ...
                        <a href="<?= htmlspecialchars(anti_xss($post['video'])) ?>" class="post-video__play-big button"
                           target="_blank">
            ...
            <footer class="post__footer">
                <div class="post__author">
                    <a class="post__author-link" href="profile.php?active_tab=posts&user_id=<?= $post['user_id'] ?>"
                       title="Автор">
...
<?php if ((int)$pages_count > 1): ?>
    <div class="popular__page-links">
        <a class="popular__page-link popular__page-link--prev button button--gray" <?= ((int)$current_page !== 1) ? 'href="popular.php?current_page='.($current_page - 1).'"' : "" ?>>
            <?= ((int)$current_page === 1) ? 'Вы находитесь на первой странице' : 'Предыдущая страница' ?></a>
        <a class="popular__page-link popular__page-link--next button button--gray" <?= ((int)$current_page !== (int)$pages_count) ? 'href="popular.php?current_page='.($current_page + 1).'"' : "" ?>>
            <?= ((int)$current_page === (int)$pages_count) ? 'Вы находитесь на последней странице' : "следующая страница" ?></a>
    </div>
<?php endif ?>
